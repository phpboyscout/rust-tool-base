...
2. **`feat(vcs): walk + diff read paths`** — async commit-walk stream, structured `Diff`. Drift-detection lands here. Originally bundled with `blame`, but `gix` 0.72 exposes blame only as the raw `gix-blame` re-export (no `Repository::blame_file` convenience) — splitting it out keeps this commit focused on the gix-Repository-method APIs and lets commit 2b decide between shelling out to `git blame` (deterministic, immediately available) or wrapping `gix-blame` directly (consistent backend, more work).
3. **`feat(vcs): blame read path`** — `Repo::blame(path, revspec) -> Blame`. Backend choice (git CLI vs `gix-blame`) is a 2b-internal decision; the wrap-not-leak A8 contract means the choice doesn't reach the public surface.
4. **`feat(vcs): clone + commit write paths`** — `Repo::clone` (HTTPS auth via `rtb-credentials`, SSH via host agent), `Repo::commit` (path-staging + message). Dirty-tree guards.
