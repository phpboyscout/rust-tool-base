...
4. **`feat(vcs): clone + commit write paths`** — `Repo::clone` (anonymous in this commit; HTTPS auth via `rtb-credentials` deferred to commit 5 alongside `fetch` where the credentials-helper plumbing exists for both ops), `Repo::commit` (path-staging + message; backend shells out to `git` per A8 wrap-not-leak — `CloneOptions` + `Repo::commit` signature are forward-compatible).
5. **`feat(vcs): fetch + checkout auth-bearing write paths`** — `Repo::fetch`, `Repo::checkout`. Dirty-tree guards apply to checkout.
6. **`feat(vcs): push via git2-fallback`** — `Repo::push` gated on the `git2-fallback` Cargo feature. CI matrix lane added for the fallback-enabled build. (Split so the libgit2 dependency lands cleanly in one commit and tools that want the rest of v0.5 without push can land it without git2 noise.)
7. **`docs: rtb-vcs v0.2 docs + framework-spec §9 alignment`** — component page extension, concept page, spec amendment.
