...
3. **`feat(vcs): blame read path`** — `Repo::blame(path, revspec) -> Blame`. Backend chose `gix-blame` directly (consistent backend; the wrap-not-leak A8 contract means the choice doesn't reach the public surface — we can swap to a shell-out fallback later without breaking consumers).
