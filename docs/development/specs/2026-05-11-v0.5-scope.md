...
Per A3 resolution = (c), six commits with feature-flag boundaries between them so partial progress always passes CI:

1. **`feat(vcs): Repo skeleton + RepoError + auth foundation`** — `git/mod.rs` with `Repo::init` + `Repo::open` + `Repo::status`, `RepoError`, auth integration with `rtb-credentials` per A2 resolution. Test against fixture repos per A5. (`init` + `open` go together because they're both constructors and the scaffolder needs `init` from day one.)
2. **`feat(vcs): walk + diff + blame read paths`** — async commit-walk stream, structured `Diff`, `Blame`. Drift-detection lands here.
3. **`feat(vcs): clone + commit write paths`** — `Repo::clone` (HTTPS auth via `rtb-credentials`, SSH via host agent), `Repo::commit` (path-staging + message). Dirty-tree guards.
4. **`feat(vcs): fetch + checkout auth-bearing write paths`** — `Repo::fetch`, `Repo::checkout`. Dirty-tree guards apply to checkout.
5. **`feat(vcs): push via git2-fallback`** — `Repo::push` gated on the `git2-fallback` Cargo feature. CI matrix lane added for the fallback-enabled build. (Split from 4 so the libgit2 dependency lands cleanly in one commit and tools that want the rest of v0.5 without push can land it without git2 noise.)
6. **`docs: rtb-vcs v0.2 docs + framework-spec §9 alignment`** — component page extension, concept page, spec amendment.
