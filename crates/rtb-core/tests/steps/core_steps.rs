...
fn given_metadata_github(world: &mut CoreWorld, owner: String, repo: String) {
    world.metadata = Some(
...
fn when_yaml_roundtrip(world: &mut CoreWorld) {
    let m = world.metadata.as_ref().expect("metadata not built");
...
fn then_release_host_is(world: &mut CoreWorld, expected: String) {
    let m = world.metadata.as_ref().expect("metadata not built");
...
fn then_name_is_also(world: &mut CoreWorld, expected: String) {
    let m = world.metadata.as_ref().expect("metadata not built");
    assert_eq!(m.name, expected);
}
...
fn given_default_features(world: &mut CoreWorld) {
    world.features = Some(Features::default());
}

#[when(regex = r#"^I disable "([^"]+)"$"#)]
fn when_disable(world: &mut CoreWorld, feature: String) {
...
fn when_enable(world: &mut CoreWorld, feature: String) {
    let f = world.features.take().expect("features not set");
    let mut builder = FeaturesBuilder::none();
    for enabled in f.iter() {
        builder = builder.enable(enabled);
...
fn then_feature_enabled(world: &mut CoreWorld, feature: String) {
...
fn then_feature_not_enabled(world: &mut CoreWorld, feature: String) {
    let f = world.features.as_ref().expect("features not set");
    assert!(!f.is_enabled(parse_feature(&feature)), "{feature} expected disabled");
}

#[then(regex = r#"^"([^"]+)" is still enabled$"#)]
fn then_feature_still_enabled(world: &mut CoreWorld, feature: String) {
...
fn given_registered_command(world: &mut CoreWorld, _name: String) {
...
impl Command for BddTestCmd {
    fn spec(&self) -> &CommandSpec {
        static SPEC: CommandSpec = CommandSpec {
            ...
            about: "registered from rtb-core's BDD binary",
            aliases: &[],
            feature: None,
        };
        &SPEC
...
fn then_is_not_dev(world: &mut CoreWorld) {
    let v = world.version.as_ref().expect("no version");
...
fn then_result_is_err(world: &mut CoreWorld) {
    let res = world.last_result.as_ref().expect("no result");
    assert!(res.is_err(), "expected Err, got {res:?}");
}

#[then(regex = r#"^the rendered diagnostic contains "([^"]+)"$"#)]
fn then_diagnostic_contains(world: &mut CoreWorld, needle: String) {
    let res = world.last_result.as_ref().expect("no result");
...
fn given_yaml(world: &mut CoreWorld, raw: String) {
...
fn when_deserialise_releasesource(world: &mut CoreWorld) {
...
fn then_github_with_host(world: &mut CoreWorld, expected: String) {
